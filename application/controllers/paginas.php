...
class Paginas extends CI_Controller {
  ...
  public function index() {
    ...
    $config['base_url'] = base_url().'admin/Paginas';
		$config['total_rows'] = $this->Paginas_model->count_paginas();
		...
		$data["paginas"] = $this->Paginas_model->get_paginas($maximo,$inicio);
    $data['main_content'] = 'admin/paginas/list';
    ...
    if ($this->input->server('REQUEST_METHOD') === 'POST') {
      ...
      $this->form_validation->set_rules('descricao', 'Descrição', 'trim|required|min_length[4]|max_length[300]');
        ...
        if (!$this->upload->do_upload('imagem')) {
          ...
          if($query = $this->Paginas_model->create($data_insert)) {
  ...
  public function update() {
      ...
      $this->form_validation->set_rules('descricao', 'Descrição', 'trim|required|min_length[4]|max_length[300]');
        ...
        if (!empty($_FILES["imagem"]['name'])) {
          ...
          if (!$this->upload->do_upload('imagem')) {
            ...
            unlink("./uploads/".$this->Paginas_model->get_imagem_by_id($id));
            $file_data = $this->upload->data();
            $data_update = array(
              'titulo_br' => $this->input->post('titulo_br'),
              'titulo_en' => $this->input->post('titulo_en'),
          ...
          $data_update = array(
            ...
            'data' => date("Y-m-d H:i:s")
				...
				if($query = $this->Paginas_model->update($id,$data_update)) {
    ...
    $data['paginas'] = $this->Paginas_model->get_pagina_by_id($id);
    $data['main_content'] = 'admin/paginas/edit';
    $this->load->view('includes/admin_template', $data);
  }
  ...
  public function delete() {
		$id = $this->uri->segment(4);
    unlink("./uploads/".$this->Paginas_model->get_imagem_by_id($id));
		if ($this->Paginas_model->delete($id)) {
